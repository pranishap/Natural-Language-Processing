...
def evaluate_combinations(X_train, Y_train, X_test, Y_test, model, w2v_vector):
   ...
   for i,each_comb in enumerate(comb_list):
    ...
    clf = 0
    if model == "SVM":
       clf = SVC(kernel='linear',C=0.1,random_state=123,class_weight="balanced",max_iter=100,probability=True)
    ...
    Y_preds = clf.predict(X_test_v.toarray())
    mistach_index = []
    mismatch_X = []
    for i,pred_val in enumerate(Y_preds):
        
        if(pred_val!= Y_test[i]):
            print(pred_val)
            print(X_test[i])
            mistach_index.append(i)
            #print(X_test[i])
            mismatch_X.append(X_test_v[i])
    probas = clf.predict_proba(X_test_v.toarray())
    newprobs =[]
    #print(clf.coef_.flatten())
    #print(vec.get_feature_names())
    zipped = zip(vec.get_feature_names(),clf.coef_.flatten())
    newZipped = sorted(zipped, key=lambda x: x[1])
    #print(newZipped)
    zipdict = dict(newZipped)
    mistachtoken = []
    for index in mistach_index:
        #print(probas[index])
        #print(X_test_v[index])
        if probas[index][0] > 0.7 or probas[index][1] > 0.7:
            multiplyresult = []
            xmat = X_test_v.todense()
            ar = np.asarray(X_test_v[index,:].todense())
            #print(ar[0])
            for a,b in zip(ar[0],clf.coef_[0]):
                multiplyresult.append(a*b)
            finalzip = zip(vec.get_feature_names(),multiplyresult)
            finalzipsorted = sorted(finalzip, key=lambda x: x[1])
            #print(finalzipsorted[-10:])
            slist = finalzipsorted[-10:]
            for sl1,sl2 in slist:
                mistachtoken.append(sl1)
    count = Counter(mistachtoken)
    print(count)
